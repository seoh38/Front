...
//     } (10, 20) // 자동으로 실행되기 때문에 매개값을 주어야 한다.
// );
...
// 매개변수 기본값을 지정하면 전달되는 매개값이 없을 경우 기본값으로 사용한다.(ES6부터 도입)
...
function thisTest(element) { 
    console.log(this); // 함수내에서 this를 찍으면 window를 가리킨다.(함수, 전역변수는 window에 포함된다.)
    console.log(element); // element는 이벤트가 발생했을때의 이벤트가 발생한 요소
}
...
btn4.addEventListener('click', () => {
    ...
    returnTest2()(); // 리턴되는 함수를 바로 실행하는 구문
});

function returnTest2() {

    // 클로저 ?? 뭔소리여
    //   - 내부함수가 사용하는 외부함수의 지역변수는 내부함수가 소멸할 때까지 소멸되지 않는 특성을 클로저 라고 한다.
    let userName = document.getElementById('userName').value;

    return function() { // 익명함수 function
