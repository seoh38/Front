...
btn1.addEventListener('click', () => {
    ...
    String 객체에서 정규 표현식 객체를 이용하는 메소드
        ...
        - 문자열.replace(정규식, 바꿀값) : 문자열에 정규식 패턴의 값과 일치하는 부분을 바꿀값으로 변경한다.
        - 문자열.search(정규식) : 문자열에서 전규식 패턴의 값과 일치하는 부분의 시작 인덱스를 리턴한다.
    ...
    */
    ...
    area.innerHTML += `str.serach(regExp) : ${str.search(regExp)} <br>`;
...
btn3.addEventListener('click', () => {
    ...
    // ?는 앞선 패천이 최대 한번이상 반복되는 문자열을 의미한다.
    str = 'a aa aaa aaaa';
    regExp = /a{1,2}/g;
    ...
    // 최소 2개만 반복이 만족되면 된다.
    regExp = /a{2,}/g; // 앞선 패턴이 최소 두번이상 반복되는 문자열을 의미한다.
    area.innerHTML += `/a{2,}/g : ${str.replace(regExp, '($&)')} <br>`;
    
    // (a) (aa) (aaa) (aaaa)
    regExp = /a+/g; // {1, } 하나 이상 반복되면 변경
    area.innerHTML += `/a{2+}/g : ${str.replace(regExp, '($&)')} <br>`;

    // (a)() (a)(a)() (a)(a)(a)() (a)(a)(a)(a)()
    regExp = /a?/g; // {0, 1} 최대 한번 이상 반복되는 문자열을 치환
    area.innerHTML += `/a{2?}/g : ${str.replace(regExp, '($&)')} <br><br>`;
    ...
    regExp = /ipt$/igm;
    area.innerHTML += `/jpt$/ig : ${str.replace(regExp, '($&)')} <br>`;

    // OR 검색(둘중에 하나만 만족하는 경우)
    // [...] 안에 문자중 하나라도 존재할 경우 
    ...
    // a 와 j 빼고 다 치환??
    // [^..] : [] 내의 ^는 not 의 의미를 가진다.(주의)
    regExp = /[^aj]/ig;
    area.innerHTML += `/[aj]/ig : ${str.replace(regExp, '($&)')} <br>`;
    
    str = '123 Javascript';
    regExp = /[a-z]/g; // 범위를 지정하려면 [...] 내의 -를 사용한다. (a~z)
    ...
    regExp = /[0-9A-Z]/g; // 띄어쓰기나 , 사용하지 않고 붙여서 사용!!
    area.innerHTML += `/[0-9A-Z]/g : ${str.replace(regExp, '($&)')} <br>`;

    // 임의의 문자열 검색
    // .은 임의의 문자 한개를 의미하고, 문자내용은 무엇이든 상관없다.
    // 공백도 하나의 문자를 의미한다.
    regExp = /..../g;
...
btn4.addEventListener('click' , () => {
    ...
    /*
        ...
        \s : 공백 문자를 의미하다.(띄어쓰기, 탭, 줄바꿈)
    */ 
    str = 'A1 B2 C3 d_4 e:5 ` ffgg77--__-- \t가\n나\n다';
    regExp = /\d/g; // [0-9]와 결과가 같다.

    area.innerHTML = '<h3>추가 메타 문자</h3>'; 
    area.innerHTML += `/\\d/g : ${str.replace(regExp, '($&)')} <br>`;
    
    regExp = /\D/g; // /[^0-9]/g와 와 결과가 같다.
    area.innerHTML += `/\\D/g : ${str.replace(regExp, '($&)')} <br>`;

    regExp = /\w/g; // /[a-zA-Z0-9_]/g 와 와 결과가 같다.
